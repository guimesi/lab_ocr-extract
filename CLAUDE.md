...
**Domain extraction guides** live in `prompts/*.md` (English, like all prompts) and are offered in a selectbox when structured mode is on — a guide whose stem matches the loaded table name is preselected, else the single available guide. A guide teaches the model how a document type maps onto the columns (e.g. cross-multiplying a WBS matrix diagram into rows) and may add extra keys beyond the schema — `parse_rows_json()` keeps extras after the schema columns. The result view can split into one table per value of a column (tabs + per-table CSV, one Excel sheet each); a column named `*_VERSION` with >1 distinct value is auto-selected, which is how multi-version documents (original vs. restart WBS) become separate tables. The uploaded file name is passed into the extraction prompt (file names often encode ids).
